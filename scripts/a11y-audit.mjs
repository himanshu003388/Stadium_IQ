...
async function runAudit() {
  ...
  for (const page of PAGES) {
    ...
    try {
      ...
      console.log(`  → ${violations.length} violations (${results[results.length - 1].criticalCount} critical, ${results[results.length - 1].seriousCount} serious, ${results[results.length - 1].moderateCount} moderate, ${results[results.length - 1].minorCount} minor)`);
    } catch (err) {
  ...
  <div class="container">
    ...
    ${results.map((r) => `
      <div class="page-card">
        ...
        ${r.violationCount === -1 ? '' : r.violationCount === 0 ? '<p class="empty">✅ No violations found</p>' : `
